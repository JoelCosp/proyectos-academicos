<template>
    ...
    <div class="container-fluid d-flex justify-content-center search-content-view">
      <div class="container m-0 p-0 homeContainer">
        <h1 class="px-2">{{ $t('series_view') }}</h1>
  ...
  const fetchFilmsByCategory = (categoryId) => {
    ...
    axios.get('/api/series/category/' + categoryId)
      .then(response => {
...
const active = ref(0);
const episodesBySeason = ref({media:[]});
const episodes = ref([]);
const review = ref({});
const reviews = ref([]);
